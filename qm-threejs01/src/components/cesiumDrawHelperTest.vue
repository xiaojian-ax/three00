  ...
  <div>
      ...
      <div class="toolbar-search">
       ...
       <div class="layoutinput">
        ...
        <div class="layoutinput"  style="width:60%;">
           <el-input
            type="textarea"
...
export default {
  ...
  watch: {
      x1(value){ 
        this.changeBillboard(1,value,this.y1,this.z1);
      },
      y1(value){
          this.changeBillboard(1,this.x1,value,this.z1);
      },
      z1(value){
         this.changeBillboard(1,this.x1,this.y1,value);
      }
  },
  methods:{
    changeBillboard:function(id,x,y,z){
        var entity = this.viewer.entities.getById(id);
        if(x && y && z){
            entity.position = new Cesium.Cartesian3(x-0, y-0,z-0);
   ...
   LonlatPointsTerrainData:function (lonlats, callback) {
    ...
    drawEllpse:function(x,y,z,radius){
            ...
            var lat=Cesium.Math.toDegrees(cartographic1.latitude);//cartograhphic.latitude(弧度)并非我们想要的  转化后的才是我们想要的（经纬度），
            var lng=Cesium.Math.toDegrees(cartographic1.longitude);
            var alt=cartographic1.height;
            ...
            this.viewer.entities.add({
                ...
                ellipsoid : {
                    ...
                    maximumCone :  Cesium.Math.toRadians(45),
       ...
       initTral:function(){
          ...
          if(this.path == ""){
                ...
                for(var i=0;i<3*4;i++){
                    ...
                    if(i%4==0){
                        this.path1.push(i/4*150);
                    }else if(i%4==1){
                        this.path1.push(px[(i-1)/4])
                    }else if(i%4==2){
                        this.path1.push(py[(i-2)/4])
                    }else{
                        this.path1.push(pz[(i-3)/4])
                    }
                //  path1.push(this.path[i]);
                }
              ...
              this.path1 = this.path.split(',').map(Number);
          }
         ...
         //this.drawDataPost();
               var  czml=[
                ...
                },
                {
                    ...
                    "model":{
                    "gltf":"/static/sampledata/model/Cesium_Air.glb",
                    ...
                    "position":{
                        ...
                        "epoch":"2019-08-04T16:00:00Z",
                        "cartesian":this.path1
                }
        }
        ];
        this.path = this.path1.join(",");
  ...
  mounted () {
        ...
        myevent.setInputAction(function(movement){
            ...
            if (pickedObject && pickedObject.id &&pickedObject.id.billboard && Cesium.defined(pickedObject) && (pickedObject.id.id === 1 || pickedObject.id.id === 2)) {
        ...
        toolbar.addListener('circleCreated', function(event) {
            loggingMessage('Circle created: center is ' + event.center.toString() + ' and radius is ' + event.radius.toFixed(1) + ' meters');
                  //世界坐标转(笛卡尔坐标)换为经纬度
            that.drawEllpse(event.center.x,event.center.y,event.center.z,event.radius.toFixed(1));
          ...
          //  circle.setEditable();
            ...
            //     loggingMessage('Circle edited: radius is ' + event.center.toString() + ' and radius is ' + event.radius.toFixed(1) + ' meters');
            // });
        });
